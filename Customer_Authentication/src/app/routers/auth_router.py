...
def login(email: str = Form(), pword: str = Form()):
    ...
    if jwt_token:
        ...
        response.set_cookie(key="login_token", value=jwt_token, httponly=True, samesite="None", path="/", expires=expires, max_age=TOKEN_EXPIRATION_TIME)
        
        user = get_user(email)
        user_name = user.firstName if user else "Guest"
        response.set_cookie(key="user_name", value=user_name, httponly=True, samesite="None", path="/", expires=expires, max_age=TOKEN_EXPIRATION_TIME)
        return response
    else:
        return JSONResponse(
...
async def forgot_password(response: Response, request: Request, email: str = Form(...)):
    if handle_forgot_password(email):
        ...
        response.set_cookie(key="verif_code", value=code_token, httponly=True, samesite="None", path="/", expires=expires, max_age=TOKEN_EXPIRATION_TIME)
        response.set_cookie(key="verif_email", value=email, httponly=True, samesite="None", path="/", expires=expires, max_age=TOKEN_EXPIRATION_TIME)
...
def manager_login(manager_id: str = Form(), password: str = Form()):
    ...
    if jwt_token:
        ...
        response.set_cookie(key="manager_login_token", value=jwt_token, httponly=True, samesite="None", path="/", expires=expires, max_age=TOKEN_EXPIRATION_TIME)

        manager = get_manager(manager_id)
        manager_name = manager.firstName if manager else "Manager"
        response.set_cookie(key="manager_name", value=manager_name, httponly=True, samesite="None", path="/", expires=expires, max_age=TOKEN_EXPIRATION_TIME)
